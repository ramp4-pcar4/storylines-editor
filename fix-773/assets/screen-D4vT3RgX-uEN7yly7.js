import{fv as O,hk as X,fC as j,fZ as U,fA as _,fB as R,gc as v,fD as V,fw as D,g0 as F,fG as q,hT as B,f_ as Q,fQ as z,fM as n,fV as k,fL as d,f$ as f,g4 as c,fY as u,h4 as Y}from"./main-D8UjLOIQ.js";import{u as M}from"./marked.esm-DgqJBp_S-BHXu-cO7.js";const G=`<?xml version="1.0" encoding="UTF-8"?>
...
`,W={key:0},ee={class:"flex justify-center"},te={key:0,class:"flex flex-col justify-center text-center"},ae=["innerHTML"],le=["innerHTML"],se={key:3,class:"flex flex-col justify-center text-center"},oe={class:"text-xl mt-20"},re={key:1,class:"p-5"},de=O({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(i){const s=X(),{t:r}=j(),x=U("iApi"),I=_(),a=i,w=R(()=>s.status),y=R(()=>s.response),P=_(!1),g=v({}),C=v([]),T=v([]);V(()=>{s.status="loading",$(),C.push(x.event.on(D.LAYER_REMOVE,e=>{a.payload.layer?.uid===e.uid&&a.panel.close()})),T.push(F(()=>a.payload.layer.uid,(e,t)=>{e!==t&&(s.status="loading",$())}))}),q(()=>{C.forEach(e=>x.event.off(e)),T.forEach(e=>e())});const $=()=>{P.value=a.payload.layer!==void 0&&!a.payload.layer.isRemoved,a.payload.type==="xml"?N(a.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(p(`${e.firstElementChild.outerHTML}`)),(a.payload.catalogueUrl||a.payload.url)&&t.appendChild(p(`<h5 class="text-xl font-bold mb-3">${r("metadata.xslt.metadata")}</h5>`)),a.payload.catalogueUrl&&t.appendChild(p(`<p><a style="color: blue;" href="${a.payload.catalogueUrl}" target="_blank">${r("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(p(`<p><a style="color: blue;" href="${a.payload.url}" target="_blank">${r("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):a.payload.type==="html"?b(a.payload.url).then(e=>{s.status=e.status,s.response=e.response}):a.payload.type==="md"&&b(a.payload.url).then(e=>{s.status=e.status,s.response=M(e.response)})},N=(e,t)=>{let l;return a.payload.xmlType&&a.payload.xmlType==="DCAT"?l=x.language==="en"?Z:J:l=x.language==="en"?G:K,l=l.replace(/\{\{([\w.]+)\}\}/g,(o,m)=>r(m)),g[e]?Promise.resolve(S(g[e],l,t)):b(e).then(o=>(g[e]=o.response,S(g[e],l,t)))},S=(e,t,l)=>{let o=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,L=new DOMParser,E=L.parseFromString(e,"text/xml"),A=L.parseFromString(t,"text/xml");m.importStylesheet(A),l&&l.forEach(h=>m.setParameter("",h.key,h.value||"")),o=m.transformToFragment(E,document);const H={className:"underline text-blue-700 break-all",target:"_blank",validate:{url:h=>/^https?:\/\//.test(h)}};a.payload.treatXmlAsMarkdown?o.firstChild.innerHTML=M.parse(o.firstChild.innerHTML):o.firstChild.innerHTML=B(o.firstChild.innerHTML,H)}return o},b=e=>new Promise(t=>{const l=new XMLHttpRequest;l.open("GET",e,!0),l.responseType="text",l.onload=()=>{l.status===200?t({status:"success",response:l.response}):t({status:"error",response:"Could not load results from remote service."})},l.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},l.send()});function p(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const l=Q("panel-screen");return n(),z(l,{panel:i.panel,ref_key:"el",ref:I},{header:k(()=>[Y(c(u(r)("metadata.title"))+": "+c(i.payload.layerName),1)]),content:k(()=>[P.value?(n(),d("div",W,[f("div",ee,[w.value=="loading"?(n(),d("div",te,c(u(r)("metadata.loading")),1)):i.payload.type==="xml"&&w.value=="success"?(n(),d("div",{key:1,innerHTML:y.value,class:"flex flex-col justify-center max-w-full xml-content"},null,8,ae)):(i.payload.type==="html"||i.payload.type==="md")&&w.value=="success"?(n(),d("div",{key:2,innerHTML:y.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,le)):(n(),d("div",se,[t[0]||(t[0]=f("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),f("span",oe,c(u(r)("metadata.error")),1)]))])])):(n(),d("div",re,[f("span",null,c(u(r)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{de as default};
