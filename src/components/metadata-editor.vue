...
export default class MetadataEditorV extends Vue {
    ...
    created(): void {
        ...
        if (this.$route.name === 'editor') {
            ...
            if (props && props.configs && props.configFileStructure && this.lockStore.uuid) {
                console.log('props passed to storyline');
                console.log(props);
    ...
    generateNewConfig(): void {
        ...
        if (!this.metadata.logoName) {
            ...
            // since config is being cloned for the opposite langs config, should this be moved to the shared folder?
            // however upon uploading logo for specific langs config, it must be added to that langs asset folder, assuming
            // its not already in the opposite langs asset folder (in which case the asset is moved to the shared asset folder)
        ...
        console.log(config.introSlide.logo.src);
        config.slides = [];
    ...
    incrementSourceCount(src: string): void {
        console.log(' ');
        console.log('incrementing source count of:');
        console.log(src);
    ...
    updateToSharedAsset(oppositeAssetPath: string, sharedAssetPath: string, oppositeLang: string): void {
        console.log(' ');
        console.log('updateToSharedAsset()');
     ...
     * @param file The file for which we are looking for duplicates within the specified folder
     * @param folder The asset folder to search for the specified asset file
     * @param checkNested Whether we should search subfolders within specfied asset file
     */
    ...
    /**
     * Helper used to upload a new logo for the current langs config. Only needed when uploading a new logo to an
     * existing config (not when creating a new product)
     * @param logo the logo being uploaded
     * @param config the config of the lang within which the logo was uploaded
     */
    async uploadLogo(logo: File, config): Promise<void> {
        console.log(' ');
        ...
        const duplicatesInShared = await this.filesInAssetFolder(logo, 'shared', false);
        let inSharedAsset = false;
        let oppositeSourceCount = 0;
        let newAssetName = logo.name;
        let uploadSource = `${this.configFileStructure.uuid}/assets/shared/${logo.name}`;
        console.log('uploadSource(before)');
        ...
        sharedAssetPaths.forEach((sharedAssetPath) => {
            // If an asset with the same name, but different content, is already in the shared
            // folder, we must give the asset we are uploading a unique name. Otherwise the existing
            // asset will be overwritten
            if (!sharedAssetPath) {
                console.log('asset with same name but different content in shared asset folder');
                let i = 2;
                while (this.configFileStructure.assets['shared'].file(newAssetName)) {
                    newAssetName = `${logo.name.split('.').at(0)}(${i}).${logo.name.split('.').at(-1)}`;
                    console.log('new unique shared asset name');
                    console.log(newAssetName);
                    i++;
                }
                uploadSource = `${this.configFileStructure.uuid}/assets/shared/${newAssetName}`;
            } else {
                console.log('asset with same name and content in shared asset folder');
                inSharedAsset = true;
            }
        });

        if (!inSharedAsset) {
            const duplicatesInOpposite = await this.filesInAssetFolder(logo, oppositeLang);
            ...
            oppositeAssetPaths.forEach((oppositeAssetPath) => {
                if (oppositeAssetPath) {
                    console.log('asset with same name and contents in opposite asset folder');
                    const oppositeFileSource = `${this.configFileStructure.uuid}/assets/${oppositeLang}/${oppositeAssetPath}`;
                    console.log('oppositeFileSource');
                    console.log(oppositeFileSource);
                    ...
                    this.configFileStructure.assets['shared'].file(newAssetName, logo);
        ...
        // If the asset uploaded is neither in the shared nor in the opposite lang's asset folder then we upload to the
        // current langs assets folder (if its not already there)
        if (!inSharedAsset) {
            console.log('logo neither in shared nor opposite');
            // This array will contain either 0 or 1 promise, since we don't care for nested assets here
            const duplicatesInCurrent = await this.filesInAssetFolder(logo, this.configLang, false);
            ...
            this.configFileStructure.assets[this.configLang].file(newAssetName, logo);
        }

        // Check if the logo src is the same as before. If it is, then we assume that nothing has changed, and thus no
        // need to increment source counts
        console.log('uploadSource(after)');
        console.log(uploadSource);
        if (config.introSlide.logo.src !== uploadSource) {
            console.log('logo src is new');
            this.incrementSourceCount(uploadSource);
            this.sourceCounts[uploadSource] += oppositeSourceCount;
            config.introSlide.logo.src = uploadSource; // source of logo for current config must be updated seperately
        }
    }
    ...
    configFileStructureHelper(configZip: typeof JSZip, uploadLogo?: File | undefined): Promise<void> {
        console.log(' ');
        console.log('configFileStrcutureHelper');
        ...
        // If uploadLogo is set, upload the logo to the shared asset folder (due to cloning of configs in
        // generateNewConfig)
    ...
    useConfig(config: StoryRampConfig): void {
        ...
        if (logo) {
            ...
            console.log('metadata logoname');
            console.log(this.metadata.logoName);
            ...
            if (logoFile) {
                ...
                // Fill in the field with this value whether it exists or not.
                this.metadata.logoName = logo;
                console.log('else case logo name');
                console.log(logo);
    ...
    async saveMetadata(publish = false): Promise<void> {
        ...
        if (config !== undefined) {
            ...
            if (!this.metadata.logoName) {
                ...
                // TODO: if the logo is the same as before, its source count gets incremented an additional time
                // Need to check whether a new logo image was uploaded, or it is the same as
                //
                console.log('logo image');
                console.log(this.logoImage);
                // Here a logo is being uploaded for a specific langs config (rather than both). In this case we
                // must check to see if this asset already exists in the opposite langs config, and if so, upload it
                // to the shared asset folder
                await this.uploadLogo(this.logoImage as File, config);
            } else {
                config.introSlide.logo.src = this.metadata.logoName;
            }
