...
export default class MetadataEditorV extends Vue {
    ...
    decrementSourceCount(src: string): void {
        console.log(' ');
        console.log('decrementing source count of:');
        console.log(src);
        if (this.sourceCounts[src]) {
            this.sourceCounts[src] -= 1;
        }
    ...
    // TODO: move the below 5 methods into a plugin. That way they aren't repeated in the image/video editors
    ...
    async compareFiles(file: File, compressedBinary: JSZip.JSZipObject, compressedName: string): Promise<boolean> {
        ...
        return compressedType !== 'svg'
            ? compressedBinary
                  .async('blob')
                  .then((img: Blob) => {
                      console.log('img blob');
                      console.log(img);
                      const currentFile = new File([img], compressedName);
                      console.log('image file');
                      console.log(currentFile);
                      console.log('decompressed binary file');
                      console.log(currentFile);
    ...
    async uploadLogo(logo: File, config): Promise<void> {
        ...
        const sharedAssetPaths = await Promise.all(this.filesInAssetFolder(logo, 'shared', false));
        ...
        if (!inSharedAsset) {
            const oppositeAssetPaths = await Promise.all(this.filesInAssetFolder(logo, oppositeLang));
        ...
        if (!inSharedAsset) {
            ...
            // This array will contain either 0 or 1 promise, since we don't care for nested assets here
            const currAssetPaths = await Promise.all(this.filesInAssetFolder(logo, this.lang, false));
            ...
            this.configFileStructure.assets[this.lang].file(newAssetName, logo);
        }

        // Check if the logo src is the same as before. If it is, then we assume that nothing has changed, and thus no
        // need to increment source counts. We also decrement source count of previous logo, as it has now been replaced
        ...
        if (config.introSlide.logo.src !== uploadSource) {
            ...
            console.log('logo src is same, nothing was done');
        }
    }
