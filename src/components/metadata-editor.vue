...
export default class MetadataEditorV extends Vue {
                ...
                .then(async (res: AxiosResponse) => {
                    ...
                    if (this.configFileStructure?.zip) {
                        ...
                        this.configFileStructure?.zip.folder('ramp-config/')?.forEach((relativePath, file) => {
                            configPromises.push(this.renameMapConfig(file.name));
                        });
            ...
            switch (panel.type) {
                ...
                case PanelType.Map:
                    if (panel.config && typeof panel.config === 'string') {
                        ...
                        panel.config = panel.config.replace(`/${this.uuid}-map-`, `/${this.changeUuid}-map-`);
                    }
                default:
    ...
    renameMapConfig(oldPath: string): Promise<void> {
        return new Promise((resolve) => {
            this.configFileStructure?.zip
                ...
                .then((res: string) => {
                    ...
                    const newFile = oldPath.replace(`/${this.uuid}-map-`, `/${this.changeUuid}-map-`);
