    ...
    <div class="editor-container">
        <template v-if="!loadEditor">
            <div class="">
                ...
                >
                    <div class="flex flex-1 my-2 text-2xl md:text-3xl font-bold">
                        {{ editExisting ? $t('editor.editProduct') : $t('editor.createProduct') }}
                    </div>
                    <div class="flex flex-row md:flex-col gap-3 justify-between text-right">
                        <router-link
                            :to="{
                ...
                <section class="px-10 md:px-20 py-5">
                    ...
                    <section>
                        <label class="font-bold">{{ $t('editor.uuid') }}</label>
                        ...
                        <br />
                        <label>{{ !editExisting ? $t('editor.metadata.newUuidInstructions') : '' }}</label>

                        <div class="pb-5 pt-2">
                            <div v-if="renaming" class="flex flex-row items-center w-full md:w-3/4">
                                <label class="mr-6 ml-3"> {{ $t('editor.uuid.new') }}: </label>
                                <input
    ...
    async generateRemoteConfig(): Promise<void> {
        ...
        try {
            ...
            if (err.name === 'AbortError' || err instanceof PointerEvent) {
                Message.success(`Product retrieval manually aborted.`);
            } else {
