...
export default class MetadataEditorV extends Vue {
    ...
    async uploadLogo(logo: File, config: StoryRampConfig): Promise<void> {
        ...
        if (!inSharedAsset) {
            ...
            const currAssetPaths = await Promise.all(this.filesInAssetFolder(logo, this.configLang, false));
        ...
        // Check if the logo src is the same as before. If it's not, then we increment the source count of the new
        // logo source. We also decrement source count of previous logo, as it has now been replaced
