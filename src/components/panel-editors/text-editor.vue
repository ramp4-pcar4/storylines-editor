<template>
    <div class="flex flex-col mt-4">
        ...
        <widget-modal @ok="(audioSettings: AudioSettings) => addAudioWidget(audioSettings)" />
    </div>
</template>
...
import type { AudioSettings, TextPanel } from '@/definitions';
...
export default class TextEditorV extends Vue {
    ...
    addAudioWidget(audioSettings: AudioSettings): void {
        this.editor.insert((selected: string) => {
            const content = `<AudioPlayer src='${audioSettings.source.name}' title='${audioSettings.title}' transcript='${audioSettings.transcript}'></AudioPlayer>`;
            return {
                text: content,
                selected: content
            };
