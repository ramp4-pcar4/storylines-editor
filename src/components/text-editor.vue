...
export default class TextEditorV extends Vue {
    ...
    toolbarTooltipAdjust(toggle): void {
        ...
        if (scrollbarVisible && toggleToRightBorder <= 90) {
            toggle.children[0].style.right = `${Math.min(
                -15,
                toggle.getBoundingClientRect().right -
                    slideEditor.getBoundingClientRect().right +
                    document.documentElement.clientWidth * 1.8 * 0.01
            )}px`;
        } else {
            toggle.children[0].style.right = `${
                toggle.getBoundingClientRect().right - slideEditor.getBoundingClientRect().right
            }px`;
        }
    }
