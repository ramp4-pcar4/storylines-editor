...
export default class EditorV extends Vue {
    ...
    created(): void {
        ...
        console.log(this.editorStore.loadStatus);
    ...
    beforeRouteUpdate(to: RouteLocationNormalized, from: RouteLocationNormalized, next: () => void): void {
        ...
        // Both `to` and `from` are guaranteed to have a uuid param, unlike the `metadata-editor` component
    ...
    beforeWindowUnload(e: BeforeUnloadEvent): void {
        ...
        if (this.editorStore.editorUnsavedChanges && !window.confirm()) {
            e.preventDefault();
        }
    }
}
