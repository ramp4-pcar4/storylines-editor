...
export default class MetadataEditorV extends Vue {
            ...
            .then((res: any) => {
                ...
                if (this.config) {
                    ...
                    if (!this.editExisting) {
                        this.loadEditor = true;
                        this.updateEditorPath(`editor-main`);
                    }
                }
            });
    }
    ...
    updateEditorPath(path: string): void {
        if (this.$route.name !== 'editor') {
            const baseUrl = window.location.href.split('/').slice(0, -1);
            const newPath = `${path}/${this.uuid}`;
            history.pushState({}, '', baseUrl.concat(newPath).join('/'));
        }
    }
}
</script>
