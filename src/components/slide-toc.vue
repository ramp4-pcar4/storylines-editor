    ...
    copyConfigFromOtherLang(index: number, currLang: keyof MultiLanguageSlide): void {
        ...
        const oppositeToSharedFolder = (panel: ImagePanel | VideoPanel, oppositeLang: string): void => {
            if (panel.src) {
                ...
                // is no asset in the shared asset folder with the same name and contents as this asset
                if (!inSharedAssets) {
                    const oppositeFileSource = panel.src;
                    const oppositeRelativePath = assetSrc.slice(3).join('/');
                    let sharedAssetName = fileName;
                    let fileZipped = this.configFileStructure.assets[oppositeLang].file(oppositeRelativePath);
                    let i = 2;
                    ...
                    while (this.configFileStructure.assets['shared'].file(sharedAssetName)) {
                        sharedAssetName = `${fileZipped.name.split('.').at(0)}(${i}).${fileZipped.name
                            .split('.')
                            .at(-1)}`;
                        i++;
                    }
                    ...
                    fileZipped.async(assetType !== 'svg' ? 'blob' : 'text').then((assetFile) => {
                        if (assetType === 'svg') {
                ...
                // At this point the src of this panel will have been updated if it was neccesary to do so
