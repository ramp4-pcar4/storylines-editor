...
app.route(ROUTE_PREFIX + '/upload').post(function (req, res, next) {
    ...
    form.parse(req, async function (err, field, file) {
        ...
        decompress(secureFilename, fileName).then(async () => {
            ...
            // Check if the latest commit has resulted in >10 commits.
            // If yes, we need to squash the first and second commits together so that the total is again 10 commits.
            // In an ideal world this would be an interactive rebase, but obivously we can't do that here.
            if (commits.all.length > 10) {
                // Reverse the commits from earliest to latest for ease of parsing.
                const allCommits = commits.all.reverse();
                const secondHash = allCommits[1].hash;
                const secondMessage = allCommits[1].message;
                const otherHashes = allCommits.slice(2).map(com => com.hash)
                // Get the current branch. We do it this way instead of assuming it as "main" in case someone has it set to master.
                const branches = await git.branchLocal()
                const currBranch = branches.current
                // Checkout to a new branch at the point of the second commit.
                await git.checkoutBranch(`squash-${secondHash}`, secondHash);
                // Reset the two commits and turn them into one commit.
                await git.reset('soft', ['HEAD~']);
                await git.raw('commit', '--amend', '--no-edit', '-m', secondMessage);
                // Now re-apply all the other commits on top
                for (let i = 0; i < otherHashes.length; i += 1) {
                    await git.raw('cherry-pick', otherHashes[i]);
                }
                // Delete the main branch and turn this into the main branch
                await git.raw('branch', '-D', currBranch)
                await git.raw('branch', '-m', currBranch)
            }
